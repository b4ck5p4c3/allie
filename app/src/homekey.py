...
def standard_auth(
    ...
    log.debug(f"authentication_hash_input={authentication_hash_input.hex()}")

    signature = reader_private_key.sign(
        authentication_hash_input, ec.ECDSA(hashes.SHA256())
    )
    log.debug(f"signature={signature.hex()} ({hex(len(signature))})")
    x, y = decode_dss_signature(signature)
    signature_point_form = bytes([*x.to_bytes(32, "big"), *y.to_bytes(32, "big")])
    log.debug(f"signature_point_form={signature_point_form.hex()} ({hex(len(signature_point_form))})")
    ...
    log.debug(f"hkdf={hkdf.hex()}")
    kenc = hkdf[: key_size * 1]
    kmac = hkdf[key_size * 1 : key_size * 2]
    krmac = hkdf[key_size * 2 :]
    log.debug(f"kenc={kenc.hex()} kmac={kmac.hex()} krmac={krmac.hex()}")
    ...
    log.debug(f"device_identifier={device_identifier.hex()}")
    ...
    log.debug(f"signature={signature.hex()}")
    ...
    log.debug(f"verification_hash_input={verification_hash_input.hex()}")
...
def perform_authentication_flow(
    ...
    log.debug(f"protocol_version={protocol_version.hex()}")
    ...
    log.debug(f"attestation_exchange_common_secret={attestation_exchange_common_secret.hex()}")
    ...
    log.debug(f"attestation_package={attestation_package}")
    ...
    try:
        ...
        log.debug("Attestation signature is invalid ")
        return DigitalKeyFlow.ATTESTATION, None, None

    log.debug(f"Attestation signature is valid {endpoint}")
...
def read_homekey(
    ...
    log.debug(f"reader_identifier={reader_identifier.hex()}")
